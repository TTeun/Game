...
{
    std::vector<Model::Geometry::Rectangle> terrainRectangles;
    ...
    const auto sizeOfObject = Model::Geometry::Point(width, height);
    for (const auto & terrainRectangle : terrainRectangles) {
        ...
        m_rectangles.push_back(Geometry::Rectangle(sizeOfObject, position - sizeOfObject).shrink(0.999f));
        m_rectangles.push_back(Geometry::Rectangle(sizeOfObject, position + size).shrink(0.999f));
        m_rectangles.push_back(
            Geometry::Rectangle(sizeOfObject, position + Model::Geometry::Point{size.x, -height}).shrink(0.999f));
        m_rectangles.push_back(
            Geometry::Rectangle(sizeOfObject, position + Model::Geometry::Point{-width, size.y}).shrink(0.999f));
    }
...
void Model::DataStructures::RectGraph::addTarget(const Geometry::Rectangle & targetRectangle,
                                                 ...
                                                 float shrinkFactor)
{
    ...
    auto * shrunkTargetRectangle = new Geometry::Rectangle(targetRectangle.shrink(shrinkFactor));
    ...
    const Geometry::Rectangle & rectangle, const Entities::LevelWrapper & levelWrapper, float shrinkFactor) const
{
    ...
    const Geometry::Rectangle shrunkRectangle = Geometry::Rectangle(rectangle.shrink(shrinkFactor));
    ...
    if (minIndex == m_rectangles.size()) {
        return (shrunkRectangle.getVectorToOtherRectangle(*m_target));
    } else {
        if (minIndex == std::numeric_limits<size_t>::max()) {
            return {0.0f, 0.0f};
        }
        return (shrunkRectangle.getVectorToOtherRectangle(m_rectangles.at(minIndex)));
    }
}
...
Model::DataStructures::RectGraph::buildDijkstraVertices() const
{
    ...
    while (true) {
        ...
        for (auto i : m_edgeArray.at(minIndex)) {
            std::pair<size_t, size_t> edgePair = std::make_pair(std::min(i, minIndex), std::max(i, minIndex));
...
std::pair<size_t, float> Model::DataStructures::RectGraph::findClosestNotYetAdded(
...
const std::vector<Model::Geometry::Rectangle> & Model::DataStructures::RectGraph::getRectangles() const
{
    return m_rectangles;
}
...
const std::unique_ptr<Model::Geometry::Rectangle> & Model::DataStructures::RectGraph::getTarget() const
{
    return m_target;
}
bool Model::DataStructures::RectGraph::hasTarget() const
{
