...
public:
    ...
    /**	The world does not agree on whether white space should be kept or
       ...
       functions are provided to set whether or not TinyXml will condense all
       white space into a single space or not. The default is to condense. Note
    ...
    /** Return the position, in the original source file, of this node or
       ...
       column. Computation can be disabled if TiXmlDocument::SetTabSize() is
       called with 0 as the value.
    ...
    virtual const char *
    Parse(const char * p, TiXmlParsingData * data, TiXmlEncoding encoding /*= TIXML_ENCODING_UNKNOWN */) = 0;
...
public:
    ...
    /** An output stream operator, for every class. Note that this outputs
       ...
       which includes tabs and new lines.

                    The operator<< and operator>> are not completely symmetric.
       Writing a node to a stream is very well defined. You'll get a nice stream
                    of output, without any extra whitespace or newlines.

                    But reading is not as well defined. (As it always is.) If
       you create a TiXmlElement (for example) and read that from an input
       stream, the text needs to define an element or junk will result. This is
                    true of all input streams, but it's worth keeping in mind.

                    A TiXmlDocument will read nodes until it reads a root
       element, and all the children of that root element.
            */
    ...
    const TiXmlNode * LastChild(const char * value) const; /// The last child of this node matching 'value'. Will be
    ...
    /** Add a new node related to this. Adds a child past the LastChild.
       ...
       efficient and avoids an extra copy, but should be used with care as it
    ...
    /** Accept a hierchical visit the nodes in the TinyXML DOM. Every node in
       ...
       it doesn't re-parse the XML for the callbacks, so the performance of
       TinyXML is unchanged by using this interface versus any other.)
...
public:
    ...
    /** QueryIntValue examines the value string. It is an alternative to the
       ...
       0, which is the opposite of almost all other TinyXml calls.
        */
...
public:
    ...
    /** Convenience function for easy access to the text inside an element.
       ...
       was created from this XML:
                @verbatim
                <foo><b>This is text</b></foo>
                @endverbatim

                then the value of str would be null. The first child node isn't
       a text node, it is another element. From this XML:
                @verbatim
...
public:
    ...
    /** Load a file using the given FILE*. Returns true if successful. Note that
       ...
       XML from the current file location. Streaming may be added in the future.
        */
    ...
    /** Get the root element -- the only top level element -- of the document.
       ...
       tolerant of multiple elements at the document level.
        */
