...
namespace Model {
    namespace DataStructures {
        ...
        RectGraph::RectGraph(float width, float height, const Entities::Level & level)
        {
            ...
            for (const auto & terrainRectangle : terrainRectangles) {
                ...
                m_rectangles.push_back(
                    Shape::Rectangle(sf::FloatRect(position - sf::Vector2f(width, height), {width, height}))
                        .shrink(0.999f));
                m_rectangles.push_back(
                    Shape::Rectangle(sf::FloatRect(position + size, {width, height})).shrink(0.999f));
                m_rectangles.push_back(
                    Shape::Rectangle(sf::FloatRect(position + sf::Vector2f{size.x, -height}, {width, height}))
                        .shrink(0.999f));
                m_rectangles.push_back(
                    Shape::Rectangle(sf::FloatRect(position + sf::Vector2f{-width, size.y}, {width, height}))
                        .shrink(0.999f));
            }
        ...
        void RectGraph::addTarget(const Shape::Rectangle & targetRectangle,
        ...
        sf::Vector2f RectGraph::findDirectionToTarget(const Shape::Rectangle & rectangle,
                                                      const Entities::Level & level,
                                                      float shrinkFactor) const
        {
            ...
            for (size_t i = 0; i != m_rectangles.size(); ++i) {
                const auto & rect1 = m_rectangles.at(i);
                if (level.intersects(Shape::Line{rect1.topRight(), shrunkRectangle.topRight()}) ||
                    level.intersects(Shape::Line{rect1.topLeft(), shrunkRectangle.topLeft()}) ||
                    level.intersects(Shape::Line{rect1.bottomRight(), shrunkRectangle.bottomRight()}) ||
                    level.intersects(Shape::Line{rect1.bottomLeft(), shrunkRectangle.bottomLeft()})) {
                    continue;
                } else {
